# University Lab Codes - OOP  
**Object-Oriented Programming Module (2025)**  
*Java Lab Sessions, Practice Codes & Concept Demonstrations*

<img src="https://img.shields.io/badge/Language-Java-blue.svg" alt="Java"> <img src="https://img.shields.io/badge/Module-OOP-brightgreen.svg" alt="OOP"> <img src="https://img.shields.io/badge/Status-Completed-success.svg" alt="Completed">

This repository contains **all the Java codes** I wrote during my **Object-Oriented Programming (OOP)** university module — including:

- Weekly lab session exercises  
- Practical assignments  
- In-class concept demonstrations  
- Extra practice codes I wrote to strengthen my understanding

It's my personal learning journal for mastering the core pillars of OOP in Java.

---

### Core OOP Concepts Covered
| Concept                  | Folder / Examples                              |
|--------------------------|-------------------------------------------------|
| Classes & Objects        | Basic class demos, constructors                |
| Inheritance              | `Unidirectional Association`, `Bidirectional Association` |
| Polymorphism             | Method overriding, runtime polymorphism        |
| Abstraction              | Abstract classes, interfaces                   |
| Encapsulation            | Private fields + getters/setters                |
| Association              | One-to-one, one-to-many relationships         |
| Aggregation & Composition| Real-world modeling examples                   |
| Exception Handling       | Try-catch, custom exceptions                   |
| Dependency               | Loose coupling examples                        |

---

### Repository Structure
